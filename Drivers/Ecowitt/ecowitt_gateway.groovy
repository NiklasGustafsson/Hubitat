...
metadata {
  ...
  definition(name: "Ecowitt WiFi Gateway", namespace: "ecowitt", author: "Niklas Gustafsson", importUrl: "https://raw.githubusercontent.com/${gitHubUser()}/${gitHubRepo()}/${gitHubBranch()}/ecowitt_gateway.groovy") {
    capability "Sensor";
  ...
  preferences {
    ...
    input(name: "rainUnit", type: "enum", title: "<font style='font-size:12px; color:#1a77c9'>Rain Measurement Units</font>", description: "<font style='font-size:12px; font-style: italic'>Units to display rain amounts</font>", options: [0:"in", 1:"mm"], multiple: false, defaultValue: 0, required: true);
    input(name: "speedUnit", type: "enum", title: "<font style='font-size:12px; color:#1a77c9'>Wind Speed Units</font>", description: "<font style='font-size:12px; font-style: italic'>Units to display wind speed</font>", options: [0:"mph", 1:"km/h", 2:"m/s", 3:"knots"], multiple: false, defaultValue: 0, required: true);
    input(name: "pressureUnit", type: "enum", title: "<font style='font-size:12px; color:#1a77c9'>Pressure Units</font>", description: "<font style='font-size:12px; font-style: italic'>Units to display atmospheric pressure</font>", options: [0:"inHg", 1:"mmHg", 2:"hPa"], multiple: false, defaultValue: 0, required: true);
...
Integer rainUnits() {
  return settings.rainUnit.toInteger();
}

// ------------------------------------------------------------

Integer windSpeedUnits() {
  return settings.speedUnit.toInteger();
}

// ------------------------------------------------------------

Integer pressureUnits() {
  return settings.pressureUnit.toInteger();
}
